...
> **Yatter 本編との関係**: Yatter 教材のメインラインは Jetpack Compose と **Navigation 3**（`NavDisplay` / `YatterNavKey`）です。この章は **Fragment + XML Navigation Component** の扱いを補足する資料であり、Compose 版の画面遷移とは別スタックの知識として読んでください。
