...
※ エミュレータの作成方法は 2_エミュレータ作成 のページを参考
...
## ハンズオン用プロジェクトを開く
インストール確認用に作成した Empty Activity プロジェクトは、ここまでの動作確認で役目を終えます。  
以降の Yatter 演習では、リポジトリに同梱されている **`template` プロジェクト** を Android Studio で開いて実装を進めます。

1. Android Studio の Welcome 画面、または `File` → `Open` から本リポジトリ内の `template` ディレクトリを開く
2. Gradle の同期が完了するまで待つ
3. エミュレータを選択し、アプリをビルド・実行できることを確認する（この時点の `MainActivity` はほぼ空のため、真っすぐな画面でも問題ありません）

![open project](../image/0/open_project.png)

`template` を開けたら、次の資料へ進んでください。
