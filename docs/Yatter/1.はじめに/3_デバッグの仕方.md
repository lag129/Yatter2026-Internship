...
### 実践的なフィルタリング例
- アプリのパッケージ名: `package:com.dmm.bootcamp.yatter`
...
ここで使われている `apply` は、インスタンスを生成しながらプロパティを設定するためのスコープ関数です。`apply` はレシーバー自身を返すため、変数に代入しながら設定できます。詳しくは [Kotlinのスコープ関数について](../../tutorial/Kotlinのスコープ関数について/1_スコープ関数とは.md) を参照してください。
